    ...
    <div className='mobile-section lg:hidden sticky'>
      ...
      <div className={`absolute h-[calc(100vh-74px)] w-[260px] shadow-sm right-0 top-full px-10 py-14 text-lg transition-all duration-300 bg-white ${T ? 'translate-x-0':'translate-x-full'}`}>
        ...
        <div className="list">
          ...
          <div>
            <Link href='/auth/login' className='px-[18px] py-[10px] rounded-[10px] hover:shadow-lg transition-all duration-300'>Login</Link>
            <Link href="/auth/signup" className='px-[18px] py-[10px] rounded-[10px] bg-[#1892C9] text-white hover:shadow-lg transition-all duration-300'>Signup</Link>
          </div>
        </div>
      </div>
    </div>
...
const Navbar = () => {
  ...
  return (
    <header className='flex justify-between items-center px-[--padding-x] py-7 text-sm shadow-2xl shadow-[#3931751A] sticky'>
      <div className="logo">
